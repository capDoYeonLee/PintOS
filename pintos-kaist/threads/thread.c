...
thread_create (const char *name, int priority, thread_func *function, void *aux) {
	...
	/* Allocate thread. */
	t = palloc_get_page (PAL_ZERO); // 커널 공간을 위한 4kb의 싱글 페이지 할당
	if (t == NULL) return TID_ERROR;

	/* Initialize thread. */
	init_thread (t, name, priority);  // 위에서 할당한 4kb의 단일 공간에 스레드 구조체를 초기화한다.
	tid = t->tid = allocate_tid (); // 스레드의 고유한 id를 할당
...
thread_unblock (struct thread *t) {
	...
	// block 상태에서 unblock이 되니까 현재 thread를 ready_list에 넣어줌
...
init_thread (struct thread *t, const char *name, int priority) {
	...
	t->priority = priority;
	t->magic = THREAD_MAGIC;  //스레드 구조체가 손상되었는지를 확인하기 위한 값

	t->init_priority = priority; // thread 초기화할 때 우선순위 default값 31
...
void thread_wakeup(int64_t current_ticks)
{
        ...
        while (curr_elem != list_end(&sleep_list))
        {
            ...
            if (current_ticks >= curr_thread->wakeup_ticks) // 깰 시간이 됐으면
            {
				...
				preempt_priority(); // ready_list 내부에서 우선순위가 높은 스레드가 선점해야 하기 때문
            }
            else
                break;
        }
